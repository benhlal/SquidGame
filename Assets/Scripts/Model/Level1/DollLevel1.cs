...
namespace Model.Level1
{
    internal enum RobotStates
    {
        Counting,
        Inspecting,
        StandBy
    ...
    public class DollLevel1 : MonoBehaviour
    {
        ...
        {
            Debug.Log("Methode: [Inspect] Comment: [Doll Inspecting moving players]  Characters in scene:[ " +
                      Characters +
        ...
        private void RandomCounting()
        {
            currentInspectionTime = startInspectionTime; // reset inspectionTime
            animator.SetTrigger("inspect"); // reset inpect to false
            audios.clip = audio1; // change audioClip
            audios.Play();
            var musicStopper = Random.value > 0.5;
            if (audios.isPlaying)
            {
                Debug.Log("Music Stopper:" + musicStopper);
                if (!musicStopper) return;
                audios.Stop();
            }
